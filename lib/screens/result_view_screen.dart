...
class ResultViewScreen extends StatefulWidget {
  ...
  const ResultViewScreen({Key? key, required this.snap}) : super(key: key);
...
class _ResultViewScreenState extends State<ResultViewScreen> {
  ...
  loadController() {
    pdfController = PdfController(
        document:
            PdfDocument.openData(InternetFile.get(widget.snap['resultUrl'])));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          "${widget.snap['semester']} - ${widget.snap['branch']}",
